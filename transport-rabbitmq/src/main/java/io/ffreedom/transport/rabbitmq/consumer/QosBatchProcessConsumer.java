        ...
        try {
            if(cacheSize.longValue() != 0) {
                ...
                if(batchResult != null && batchResult) {
                    ...
                    linkedList.clear();
                    cacheSize.reset();
                    log.info("cache clear, current size -> {}, cache -> {} , local tag -> {}"
                            , linkedList.size(), cacheSize.longValue(), lastDeliveryTag);
                }
            }
        } catch (IOException e) {
        ...
        schedule.scheduleWithFixedDelay(() -> {
            try {
                if(cacheSize.longValue() == lastSize
                    ...
                    log.info("automatic flush cache ...{} -> {} ,deliveryTag[{}]"
                            , cacheSize.longValue(), lastSize, this.lastDeliveryTag);
                    flush();
                } else {
