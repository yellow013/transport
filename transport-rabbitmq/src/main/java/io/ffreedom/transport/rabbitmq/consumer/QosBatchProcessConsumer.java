...
public class QosBatchProcessConsumer<T> extends DefaultConsumer {
    ...
    public QosBatchProcessConsumer(Channel channel, int prefetchCount, long millisSecond,
            ...
            , QueueMessageSerializable<T> serializable, RefreshNowEvent<T> refreshNowEvent, Predicate<T> filter) {
        super(channel);
    ...
    public QosBatchProcessConsumer(Channel channel, QueueMessageSerializable<T> serializable, QosBatchCallBack<List<T>> qosBatchCallBack) {
        super(channel);
        this.channel = channel;
        this.qosBatchCallBack = Objects.requireNonNull(qosBatchCallBack, "QosBatchCallBack can not be null");
        this.serializable = Objects.requireNonNull(serializable, "QueueMessageSerializable can not be null");
    ...
    private void init() {
        ...
        ThreadFactory namedThreadFactory = new ThreadFactoryBuilder()
                .setNameFormat("BatchHandlerAutoFlush-pool-%d").build();
    ...
    private void save(T t,long lastDeliveryTag){
        ...
        try {
            ...
            if(cacheSize.longValue() >= prefetchCount) {
                log.info("The message to be stored reaches the threshold[{}] -> {} ", cacheSize.longValue(), prefetchCount);
                flush();
        ...
        try {
            ...
            if(cacheSize.longValue() != 0) {
                ...
                if(batchResult != null && batchResult) {
                    ...
                    linkedList.clear();
                    cacheSize.reset();
                    log.info("cache clear, current size -> {}, cache -> {} , tag -> {}", linkedList.size(), cacheSize.longValue(), lastDeliveryTag);
                }
            }
        } catch (IOException e) {
            ...
            log.error("batch process failure", e);
        } finally {
            lock.unlock();
        }
    }
    ...
    /**
     * schedule task : 如果缓存中的数据长期没有变动,触发flush动作
     * 1. 当缓存队列中的值等于上一次定时任务触发的值,并且该值大于0,不等于rmq预取值,则认为该缓存数据长期没有变动,立即触发回调.
     * <p>
    ...
    private void automaticFlush() {
        schedule.scheduleWithFixedDelay(() -> {
            try {
                if(cacheSize.longValue() == lastSize
                        && cacheSize.longValue() > 0
                        && cacheSize.longValue() != prefetchCount) {
                    log.info("automatic flush cache ...{} -> {}", cacheSize.longValue(), lastSize);
                    flush();
                } else {
                    lastSize = cacheSize.longValue();
                }
            } catch (Exception e) {
                log.error("automatic flush execute failure", e);
            }
